...
public class BlueRingSold extends Item{

	public BlueRingSold(int x, int y, ID type) {
		super(x, y, type);
		cost=750;
		this.type=type;
		loadImage();
	...
	private void loadImage() {
		try {img=ImageIO.read(getClass().getResourceAsStream("/map/BlueRing.png"));
		}
	...
	public void update() {
		if(x==Hero.getInstance().x && y==Hero.getInstance().y && pickUpItemTimer==null){
			if(Hero.getInstance().rupee_amount>=cost){
				...
				Hero.getInstance().hasBlueRing=true;
			}
		}
		if(pickUpItemTimer!=null){
