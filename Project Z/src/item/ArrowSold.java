...
public class ArrowSold extends Item{
	public ArrowSold(int x, int y, ID type) {
		super(x, y, type);
		cost=40;
		this.type=type;
		loadImage();
	...
	public void update() {
		if(x==Hero.getInstance().x && y==Hero.getInstance().y && pickUpItemTimer==null){
			if(Hero.getInstance().rupee_amount>=cost){
				...
				Hero.getInstance().inventory_items[1][0]=new Arrow(273,98,Item.ID.Arrow);;
				Hero.getInstance().inventory_items[1][0].hasOwnership=true;
			}
		}
		if(pickUpItemTimer!=null){
