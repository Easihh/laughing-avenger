...
public class MagicalShieldSold extends Item{

	public MagicalShieldSold(int x, int y, ID type) {
		super(x, y, type);
		cost=100;
		this.type=type;
		loadImage();
	...
	private void loadImage() {
		try {img=ImageIO.read(getClass().getResourceAsStream("/map/MagicalShield.png"));
		}
	...
	public void update() {
		if(x==Hero.getInstance().x && y==Hero.getInstance().y && pickUpItemTimer==null){
			if(Hero.getInstance().rupee_amount>=cost){
				...
				Hero.getInstance().hasMagicShield=true;
			}
		}
		if(pickUpItemTimer!=null){
